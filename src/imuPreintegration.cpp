...
using gtsam::symbol_shorthand::X; // Pose3 (x,y,z,r,p,y)   这个是符号标识X
...
public:
    ...
    */
    TransformFusion()
    {
        ...
        subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("test_gtsam/mapping/odometry", 5, &TransformFusion::lidarOdometryHandler, this, ros::TransportHints().tcpNoDelay());
        // 订阅imu里程计，来自IMUPreintegration类中的发布
        subImuOdometry   = nh.subscribe<nav_msgs::Odometry>(odomTopic+"_incremental",   2000, &TransformFusion::imuOdometryHandler,   this, ros::TransportHints().tcpNoDelay());

        // 发布imu里程计，用于rviz展示
        pubImuOdometry   = nh.advertise<nav_msgs::Odometry>(odomTopic, 2000);
        // 发布imu里程计轨迹
        pubImuPath       = nh.advertise<nav_msgs::Path>    ("test_gtsam/imu/path", 1);
    }

    /**
     * 里程计对应变换矩阵，将里程计的位置和姿态转换成4x4的矩阵
    */
    ...
    /**
     * 订阅激光里程计，来自mapOptimization，获取里程计的位置和姿态
    */
    ...
    */
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if (imuTime - last_path_time > 0.1)
        {
            ...
            // geometry_msgs::PoseStamped 是 ROS 中的消息类型之一，用于表示带有时间戳的姿态信息。
...
public:
    ...
    */
    IMUPreintegration()
    {
        ...
        subOdometry = nh.subscribe<nav_msgs::Odometry>("test_gtsam/mapping/odometry_incremental", 5,    &IMUPreintegration::odometryHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    */
    ...
    /**
     * 订阅imu原始数据
     * 1、用上一帧激光里程计时刻对应的状态、偏置，然后进行初始化，之后施加从该时刻开始到当前时刻的imu预计分量，得到当前时刻的状态，也就是imu里程计
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // 用上一帧激光里程计时刻对应的状态、偏置，
        // 拿上一时刻的状态来初始化预积分器
        // 施加从该时刻开始到当前时刻的imu预计分量，得到当前时刻的状态
        ...
        // 当前激光雷达的角速度等于当前imu的角速度加上角速度的误差得到的
