...
**************************************************/ 
#include "utility.h"
#include "test_gtsam/cloud_info.h"
#include "test_gtsam/save_map.h"
...
public:
    ...
    test_gtsam::cloud_info cloudInfo;
    ...
    */
    mapOptimization()
    {
        ...
        // 发布历史关键帧里程计
        pubKeyPoses                 = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/trajectory", 1);
        // 发布局部关键帧map的特征点云
        pubLaserCloudSurround       = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/map_global", 1);
        // 发布激光里程计，rviz中表现为坐标轴
        pubLaserOdometryGlobal      = nh.advertise<nav_msgs::Odometry> ("test_gtsam/mapping/odometry", 1);
        // 发布激光里程计，它与上面的激光里程计基本一样，只是roll、pitch用imu数据加权平均了一下，z做了限制
        pubLaserOdometryIncremental = nh.advertise<nav_msgs::Odometry> ("test_gtsam/mapping/odometry_incremental", 1);
        // 发布激光里程计路径，rviz中表现为载体的运行轨迹
        pubPath                     = nh.advertise<nav_msgs::Path>("test_gtsam/mapping/path", 1);

        // 订阅当前激光帧点云信息，来自featureExtraction
        subCloud = nh.subscribe<test_gtsam::cloud_info>("test_gtsam/feature/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        // 订阅GPS里程计
        ...
        // 发布地图保存服务
        srvSaveMap  = nh.advertiseService("test_gtsam/save_map", &mapOptimization::saveMapService, this);

        // 发布闭环匹配关键帧局部map
        pubHistoryKeyFrames   = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/icp_loop_closure_history_cloud", 1);
        // 发布当前关键帧经过闭环优化后的位姿变换之后的特征点云
        pubIcpKeyFrames       = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/icp_loop_closure_corrected_cloud", 1);
        // 发布闭环边，rviz中表现为闭环帧之间的连线
        pubLoopConstraintEdge = nh.advertise<visualization_msgs::MarkerArray>("/test_gtsam/mapping/loop_closure_constraints", 1);

        // 发布局部map的降采样平面点集合
        pubRecentKeyFrames    = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/map_local", 1);
        // 发布历史帧（累加的）的角点、平面点降采样集合
        pubRecentKeyFrame     = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/cloud_registered", 1);
        // 发布当前帧原始点云配准之后的点云
        pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("test_gtsam/mapping/cloud_registered_raw", 1);
    ...
    */
     ...
     * 8、发布里程计、点云、轨迹
    */
    void laserCloudInfoHandler(const test_gtsam::cloud_infoConstPtr& msgIn)
    {
        // 当前激光帧时间戳
    ...
    */
     ...
     * 保存全局关键帧特征点集合
    */
    bool saveMapService(test_gtsam::save_mapRequest& req, test_gtsam::save_mapResponse& res)
    {
    ...
    */
    void visualizeGlobalMapThread()
    {
        ...
        test_gtsam::save_mapRequest  req;
        test_gtsam::save_mapResponse res;
